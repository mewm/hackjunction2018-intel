...
class SensorClient(Model):
    ...
    def _sensor_status(self, data):
        ...
        if prop_id == SigUnit.celsius_temperature:
            ...
            # print(value_string)
            file_to_write_to = 'temp_node_' + CommandToBeRun.model_address[-1:] + '.txt'
...
def console_handler():
    ...
    # cmd_line = 'model 0 0x0000 0xfbf105 0xc123 0 Hacker1'
    # cmd_line = 'model 0 0x1102 0x8231 0x0001 0 0x2a1f'

    cmd_generic_handler(CommandToBeRun.command_to_be_run)
    

class CommandToBeRun:
    command_to_be_run = ''
    model_address = ''

def main(command_to_run_invoked, model_address):
    CommandToBeRun.command_to_be_run = command_to_run_invoked
    CommandToBeRun.model_address = model_address
